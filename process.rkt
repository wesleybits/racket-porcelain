...
(define (tail p [out (current-output-port)])
  (let loop:tail ()
    (let ([stdout-line (sync/timeout 0.01 (read-line-evt (stdout p)))]
          [stderr-line (sync/timeout 0.01 (read-line-evt (stderr p)))])
      (when (and stdout-line (not (eof-object? stdout-line)))
        (displayln stdout-line out))
      (when (and stderr-line (not (eof-object? stderr-line)))
        (display #\u001B out)
        (display "[31m" out)
        ...
        (display "[0m" out)
        (newline out))
      (unless (and (eof-object? stdout-line)
                   (eof-object? stderr-line))
        (loop:tail))))
  p)
